...
- 新增 `bandwidth` 命令组，支持查询独享带宽与实例 EIP 带宽归属，并通过先询价、
  再确认的流程完成购买、改配、删除和共享/独享带宽切换。
- 新增 `minimax` 命令组，覆盖 MiniMax H3 视频任务创建、查询、列表、取消以及积分套餐包和
  余额查询；模型 API Key 从独立环境变量读取，创建支持幂等键与预演。
- 兼容 Typer 0.27.1 的上下文与命令类型，确保 JSON 确认保护、结构化帮助和命令统计正常工作。
